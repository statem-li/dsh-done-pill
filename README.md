...
③ **视觉风格改为 HUD 线条感（tech-line）**——外壳**全透明无填充面**（只留
1px 发光细线与元素，同参考图）、左上/右下角框括标、方形仪表状态件
（✓框/未读框）、面板标题下沿青点虚线、未读扫描光带与运行点脉冲；
**右上引线标注**：字下底边线 → 向右上的对角线 → 与文字顶部平齐的平行线，
平行线上方显示运行中数量（点击进入最新运行会话、悬停滑出任务列表）；
圆角收窄（15→4px），深/浅主题各一套 `--dpl-*` 变量，几何骨架与形变动画
节奏不变。
