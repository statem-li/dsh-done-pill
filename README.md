...
③ **视觉风格改为 HUD 线条感（tech-line）**——外壳**全透明无框无线**（只有
文字与状态件直接浮在页面上，无矩形围绕、无引线装饰）、方形仪表状态件
（✓框/未读框/运行方块）、左侧运行中计数块（点击=进入运行会话、悬停=滑出
任务列表）、面板标题下沿青点虚线、未读扫描光带与运行点脉冲；深/浅主题
各一套 `--dpl-*` 变量，几何骨架与形变动画节奏不变。
